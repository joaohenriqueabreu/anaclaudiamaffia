<!-- Original scripts -->
<!-- JS Global Compulsory -->

{{Html::script(asset('plugins/jquery/jquery.min.js'))}}
{{Html::script(asset('plugins/jquery/jquery-migrate.min.js'))}}
{{Html::script(asset('plugins/bootstrap/js/bootstrap.min.js'))}}
{{Html::script(asset('plugins/back-to-top.js'))}}
{{Html::script(asset('plugins/jquery.easing.min.js')) }}
{{--{{Html::script(asset('js/plugins/smoothScroll.js'))}}--}}
{{Html::script(asset('js/plugins/jquery.localScroll.min.js'))}}
{{Html::script(asset('plugins/pace/pace.min.js')) }}
{{Html::script(asset('plugins/modernizr.js'))}}
{{Html::script(asset('plugins/backstretch/jquery.backstretch.min.js'))}}
{{Html::script(asset('plugins/owl-carousel2/owl.carousel.min.js'))}}

{{Html::script(asset('plugins/jquery.parallax.js'))}}

{{Html::script(asset('js/custom.js'))}}
{{Html::script(asset('js/plugins/style-switcher.js'))}}

<!--[if lt IE 9]>
{{Html::script(asset('plugins/respond.js'))}}
{{Html::script(asset('plugins/html5shiv.js'))}}
{{Html::script(asset('plugins/placeholder-IE-fixes.js'))}}

        <!-- Scripts for Header -->
{{Html::script(asset('js/one.app.js'))}}
{{Html::script(asset('js/plugins/pace-loader.js'))}}
{{Html::script(asset('js/plugins/promo.js'))}}
{{Html::script(asset('js/plugins/owl-carousel2.js'))}}


{{--{{Html::script(asset('plugins/smoothScroll.js'))}}--}}